...
A Python 3 GUI application for the Tool for the Automatic Analysis of Syntactic Sophistication and Complexity (TAASSC).
This script provides a Tkinter-based interface for selecting input/output files, configuring analysis options, and running
syntactic complexity and sophistication analyses on text corpora. It supports multiple index types, output formats, and
frequency list generation. The core processing is performed in a separate thread to keep the GUI responsive.

Key Features:
- Cross-platform GUI (macOS, Windows, Linux) with platform-specific font and geometry settings.
- Allows users to select input folders, output filenames, and analysis options via checkboxes and buttons.
- Supports calculation of clause complexity, phrase complexity, syntactic sophistication, and syntactic components.
- Option to generate frequency lists with configurable minimum frequency.
- Outputs results in CSV, text, and/or XML formats as selected.
- Uses Stanford CoreNLP for syntactic parsing (via ktatk_py3.py).
- Progress and status updates are communicated to the GUI via a multiprocessing queue.
- Modular design with helper functions for safe division, dictionary counting, ratio compilation, and database operations.

Classes:
    MyApp: Main application class for the Tkinter GUI.

Functions:
    resource_path(relative): Returns the absolute path to a resource, compatible with PyInstaller.
    start_thread(def1, arg1, arg2, arg3): Starts a function in a new thread.
    main(indir, outfile, check_list): Main processing function for TAASSC analyses.

# Changes from TAASSC_1.3.8_py2.7.py:
# - Ported to Python 3: print statements, exception syntax, and string handling updated.
# - Tkinter import style updated for Python 3 (tkinter instead of Tkinter).
# - Uses queue.Queue and multiprocessing.Queue for thread/process-safe communication.
# - Unicode and string encoding issues addressed for Python 3 compatibility.
# - File and directory handling updated for cross-platform compatibility.
# - Minor GUI improvements and bug fixes for Python 3.
"""
...
ktatk = resource_path("ktatk_py3.py")
import ktatk_py3 as ktk
...
class MyApp:
    def __init__(self, parent):
        ...
        self.spacer1 = tk.Label(
            parent,
            text="Tool for the Automatic Analysis of Syntactic\nSophistication and Complexity",
            font=helv16,
            background=color,
        )
        self.spacer1.pack()
        ...
        self.cb5 = tk.Checkbutton(
            self.checkboxframe,
            text="Syntactic\nSophistication",
            variable=self.sophistication_box,
            background=color,
        )
        ...
        self.cb6 = tk.Checkbutton(
            self.checkboxframe,
            text="Syntactic\nComponents",
            variable=self.components_box,
            background=color,
        )
        ...
        self.freqframe = tk.LabelFrame(
            self.myContainer1,
            text="VAC Frequency List Generator",
            background=color,
            width="45",
        )
        ...
        self.generate = tk.Checkbutton(
            self.freqframe,
            text="Create\nNew List",
            variable=self.generate_box,
            background=color,
        )
        ...
        self.freq_label = tk.Label(
            self.freqframe, text="Minimum VAC\nFrequency:", background=color
        )
    ...
    def runprogram(self, event):
        ...
        if self.var_list[4].get() == 1:
            if (
                self.var_list[1].get() == 0
                or self.var_list[2].get() == 0
                or self.var_list[3].get() == 0
            ):
                tkMessageBox.showinfo(
                    "Component Score Error",
                    "To calculate syntactic components, you must also calculate clause complexity, phrase complexity, and syntactic sophistication indices.\n\nPlease check these boxes.",
                )
                comp_checker = "not ok"
            else:
                comp_checker = "ok"
            ...
            comp_checker = "ok"
        if self.dirname != "" and self.outdirname != "" and comp_checker == "ok":
...
def main(indir, outfile, check_list):
    ...
    def ratio_compiler(dict, list):  # compiles ratios for collexeme attracted/repelled
        ...
        for items in list:
            if items in dict:
                if dict[items][1] == "Attracted":
                    positive_count += 1
                if dict[items][1] == "Infinity":
                    positive_count += 1
                if dict[items][1] == "Repelled":
                    negative_count += 1
                if dict[items][1] == "Neg_Infinity":
                    negative_count += 1
        return safe_divide(positive_count, negative_count)

    def ratio_compiler_type(
        dict, list
    ):  # compiles ratios for collexeme attracted/repelled but each type is only counted once
        positive_count = 0
        negative_count = 0
    ...
    def ttr(list, number, dict):
        ...
        outvar = [
            safe_divide(len(set(set_list)), len(set_list)),
            safe_divide(nattest, (nattest + nnattest)),
        ]
        return outvar
    ...
    def component_dicter(
        folder,
    ):  # takes a folder of component files, turns it into dict
        dict = {}
    ...
    def freq_database_compiler(
        output_file, lem_list, vac_list, vc_list, min_freq_value
    ):
        # Start Part 3:
        ...
        for lines in db:
            ...
            update_message = (
                "Processed "
                + str(int((counter / nlines) * 100))
                + "% of constructions in corpus"
            )
            ...
            NCNL = n_constructions - (
                LF + CF - LCF
            )  # vacs that don't have the construction or the verb
    ...
    if df_check == 1:
        if clause_check == 1 or soph_check == 1 or generate_check == 1:
            ...
            data_file.write(
                "lemma\tsophistication_vac\tfilled_sophistication_vac\tclause_complexity_vac\tfilename\tsentence_text\n"
            )

        if phrase_check == 1:
            data_filename_2 = outfile[:-4] + "_phrase_database.txt"
            data_file_2 = open(data_filename_2, "w")
            data_file_2.write(
                "np_type\tnp_head\tnp_type_with_modifiers\tfilled_np\tfilename\tsentence_text\n"
            )

    if generate_check == 1:
        list_filename = outfile[:-4] + "_frequency_database.txt"
        freq_list_out = open(list_filename, "w")
    ...
    if soph_check == 1:
        ...
        root.update_idletasks()
        acad_database_file = (
            open(resource_path("acad_index_database_3.kdk"), "r").read().split("\n")
        )
        ...
        root.update_idletasks()
        news_database_file = (
            open(resource_path("news_index_database_3.kdk"), "r").read().split("\n")
        )
        ...
        root.update_idletasks()
        mag_database_file = (
            open(resource_path("mag_index_database_3.kdk"), "r").read().split("\n")
        )
        ...
        root.update_idletasks()
        fic_database_file = (
            open(resource_path("fic_index_database_3.kdk"), "r").read().split("\n")
        )
        ...
        root.update_idletasks()
        all_database_file = (
            open(resource_path("all_index_database_3.kdk"), "r").read().split("\n")
        )
    ...
    if phrase_check == 1 or clause_check == 1 or soph_check == 1 or generate_check == 1:
        ...
        for files in p_files_list:  # iterate through files
            ...
            for sentences in tree.iter("sentence"):
                ...
                for entries in cop_list:
                    ...
                    for dependencies in deps.iter("dep"):
                        if (
                            ...
                            if (
                                pos in other
                            ):  # currently this includes only base adverbs 'RB'
                        ...
                        if dependencies.get("type") not in noun_mod:
                            if (
                ...
                if (
                    clause_check == 1 or soph_check == 1 or generate_check == 1
                ):  # Clause Complexity
                    ...
                    for items in main_verbs:
                        ...
                        if (
                            ...
                            and str(pre_simple_VAC[0][0]) in gerunds
                        ):
                            # print "g skip"
                            continue
                        ...
                        if "-v_be-" in complex_VAC_string:
                            ...
                            simple_VAC_string = simple_VAC_string.replace(
                                "-v-", "-vcop-"
                            )
                            # print complex_VAC_string
                        ...
                        lemm_entry_aux = (
                            verb_form.lower() + "\t" + "-".join(simple_VAC_aux).lower()
                        )
                        lemm_prep_entry = (
                            verb_form.lower() + "\t" + "-".join(prep_VAC).lower()
                        )
                        ...
                        if "vcop" not in lemm_entry:
                            lemma_constructicon_no_vcop.append(
                                lemm_entry
                            )  # for contingency counts
                ...
                if phrase_check == 1:  # if the Phrase Complexity Box is Checked
                        ...
                        if (
                            ...
                            for dependents in dependencies.iter("dep"):
                                ...
                                if dependents.get("type") == "nsubj":
                                    ...
                                    nsubj_list.append(
                                        dependents[1].get("idx")
                                    )  # add idx of nsubj to list for later retrieval
                                    nsubj_stdev_dict[dependents[1].get("idx")] = (
                                        0  # create dict for standard deviation
                                    )
                                    nominals_const_dict[dependents[1].get("idx")] = [
                                        [
                                            dependents[1].get("idx"),
                                            dependents.get("type"),
                                            dependents[1].text,
                                        ]
                                    ]  # create dict for construction example
                                    all_nominals_stdev_dict[
                                        dependents[1].get("idx")
                                    ] = 0
                                ...
                                if dependents.get(
                                    "type"
                                ) == "pobj" or "prep" in dependents.get("type"):
                                    ##print "dependent :", dependents.get("type"), " word: ", dependents[1].text
                        ...
                        if (
                            ...
                            for dependents in dependencies.iter("dep"):
                                if "prep" in dependents.get("type"):
                                    ...
                                    type_dependent = dependents.get("type")
                                ...
                                if gov_id in nsubj_list:
                                    ...
                                    nominals_const_dict[gov_id].append(
                                        [dep_id, type_dependent, dep_text]
                                    )  # add id, dep type, word
                    ...
                    if df_check == 1:
                        for items in nominals_const_dict:
                            ...
                            filled_const = []
                            sorted_list = sorted(
                                nominals_const_dict[items], key=lambda x: int(x[0])
                            )
                    ...
                    for items in all_nominals_stdev_dict_NN:
                        all_nominals_stdev_list_NN.append(
                            all_nominals_stdev_dict_NN[items]
                        )
            ...
            if phrase_check == 1:  # if the Phrase Complexity Box is Checked
                ...
                ktk.indexer(
                    safe_divide(n_all_nominal_deps, n_all_nominals),
                    "av_nominal_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_nsubj_deps, n_all_nsubj),
                    "av_nsubj_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_nsubj_pass_deps, n_all_nsubj_pass),
                    "av_nsubj_pass_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_agents_deps, n_all_agents),
                    "av_agents_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_dobj_deps, n_all_dobj),
                    "av_dobj_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_pobj_deps, n_all_pobj),
                    "av_pobj_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_iobj_deps, n_all_iobj),
                    "av_iobj_deps",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_ncomp_deps, n_all_ncomps),
                    "av_ncomp_deps",
                    index_list,
                    header_list,
                )
                ...
                ktk.indexer(
                    safe_divide(n_all_nominal_deps_NN, n_all_nominals_NN),
                    "av_nominal_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_nsubj_deps_NN, n_all_nsubj_NN),
                    "av_nsubj_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_nsubj_pass_deps_NN, n_all_nsubj_pass_NN),
                    "av_nsubj_pass_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_agents_deps_NN, n_all_agents_NN),
                    "av_agents_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_dobj_deps_NN, n_all_dobj_NN),
                    "av_dobj_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_pobj_deps_NN, n_all_pobj_NN),
                    "av_pobj_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_iobj_deps_NN, n_all_iobj_NN),
                    "av_iobj_deps_NN",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    safe_divide(n_ncomp_deps_NN, n_all_ncomps_NN),
                    "av_ncomp_deps_NN",
                    index_list,
                    header_list,
                )

                ktk.std_dev_calc_simple(
                    all_nominals_stdev_list,
                    "nominal_deps_stdev",
                    index_list,
                    header_list,
                ...
                ktk.std_dev_calc_simple(
                    nsubj_stdev_list, "nsubj_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    nsubj_pass_stdev_list, "nsubj_pass_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    agents_stdev_list, "agents_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    dobj_stdev_list, "dobj_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    pobj_stdev_list, "pobj_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    iobj_stdev_list, "iobj_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    ncomp_stdev_list, "ncomp_stdev", index_list, header_list
                )

                ktk.std_dev_calc_simple(
                    all_nominals_stdev_list_NN,
                    "nominal_deps_NN_stdev",
                    index_list,
                    header_list,
                )
                ktk.std_dev_calc_simple(
                    nsubj_stdev_list_NN, "nsubj_NN_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    nsubj_pass_stdev_list_NN,
                    "nsubj_pass_NN_stdev",
                ...
                ktk.std_dev_calc_simple(
                    agents_stdev_list_NN, "agents_NN_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    dobj_stdev_list_NN, "dobj_NN_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    pobj_stdev_list_NN, "pobj_NN_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    iobj_stdev_list_NN, "iobj_NN_stdev", index_list, header_list
                )
                ktk.std_dev_calc_simple(
                    ncomp_stdev_list_NN, "ncomp_NN_stdev", index_list, header_list
                )

                ##### Begin Medium-Grained Index Counts ######

                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "det", n_all_nominals),
                    "det_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "amod", n_all_nominals),
                    "amod_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "prep", n_all_nominals),
                    "prep_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "poss", n_all_nominals),
                    "poss_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "vmod", n_all_nominals),
                    "vmod_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "nn", n_all_nominals),
                    "nn_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "rcmod", n_all_nominals),
                    "rcmod_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "advmod", n_all_nominals),
                    "advmod_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "conj_and", n_all_nominals),
                    "conj_and_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_dict, "conj_or", n_all_nominals),
                    "conj_or_all_nominal_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "det", n_all_nominals_NN),
                    "det_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "amod", n_all_nominals_NN),
                    "amod_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "prep", n_all_nominals_NN),
                    "prep_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "poss", n_all_nominals_NN),
                    "poss_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "vmod", n_all_nominals_NN),
                    "vmod_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "nn", n_all_nominals_NN),
                    "nn_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "rcmod", n_all_nominals_NN),
                    "rcmod_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(all_nominal_deps_NN_dict, "advmod", n_all_nominals_NN),
                    "advmod_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(
                        all_nominal_deps_NN_dict, "conj_and", n_all_nominals_NN
                    ),
                ...
                ktk.indexer(
                    dict_counter(
                        all_nominal_deps_NN_dict, "conj_or", n_all_nominals_NN
                    ),
                    "conj_or_all_nominal_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ...
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "det", n_all_nsubj),
                    "det_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "amod", n_all_nsubj),
                    "amod_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "prep", n_all_nsubj),
                    "prep_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "poss", n_all_nsubj),
                    "poss_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "vmod", n_all_nsubj),
                    "vmod_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "nn", n_all_nsubj),
                    "nn_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "rcmod", n_all_nsubj),
                    "rcmod_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "advmod", n_all_nsubj),
                    "advmod_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "conj_and", n_all_nsubj),
                    "conj_and_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "conj_or", n_all_nsubj),
                    "conj_or_nsubj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "det", n_all_nsubj_NN),
                    "det_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "amod", n_all_nsubj_NN),
                    "amod_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "prep", n_all_nsubj_NN),
                    "prep_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "poss", n_all_nsubj_NN),
                    "poss_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "vmod", n_all_nsubj_NN),
                    "vmod_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "nn", n_all_nsubj_NN),
                    "nn_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "rcmod", n_all_nsubj_NN),
                    "rcmod_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "advmod", n_all_nsubj_NN),
                    "advmod_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "conj_and", n_all_nsubj_NN),
                    "conj_and_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(nsubj_deps_dict, "conj_or", n_all_nsubj_NN),
                    "conj_or_nsubj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "det", n_all_dobj),
                    "det_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "amod", n_all_dobj),
                    "amod_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "prep", n_all_dobj),
                    "prep_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "poss", n_all_dobj),
                    "poss_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "vmod", n_all_dobj),
                    "vmod_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "nn", n_all_dobj),
                    "nn_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "rcmod", n_all_dobj),
                    "rcmod_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "advmod", n_all_dobj),
                    "advmod_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "conj_and", n_all_dobj),
                    "conj_and_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "conj_or", n_all_dobj),
                    "conj_or_dobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "det", n_all_dobj_NN),
                    "det_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "amod", n_all_dobj_NN),
                    "amod_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "prep", n_all_dobj_NN),
                    "prep_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "poss", n_all_dobj_NN),
                    "poss_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "vmod", n_all_dobj_NN),
                    "vmod_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "nn", n_all_dobj_NN),
                    "nn_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "rcmod", n_all_dobj_NN),
                    "rcmod_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "advmod", n_all_dobj_NN),
                    "advmod_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "conj_and", n_all_dobj_NN),
                    "conj_and_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(dobj_deps_dict, "conj_or", n_all_dobj_NN),
                    "conj_or_dobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "det", n_all_pobj),
                    "det_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "amod", n_all_pobj),
                    "amod_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "prep", n_all_pobj),
                    "prep_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "poss", n_all_pobj),
                    "poss_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "vmod", n_all_pobj),
                    "vmod_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "nn", n_all_pobj),
                    "nn_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "rcmod", n_all_pobj),
                    "rcmod_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "advmod", n_all_pobj),
                    "advmod_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "conj_and", n_all_pobj),
                    "conj_and_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "conj_or", n_all_pobj),
                    "conj_or_pobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "det", n_all_pobj_NN),
                    "det_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "amod", n_all_pobj_NN),
                    "amod_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "prep", n_all_pobj_NN),
                    "prep_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "poss", n_all_pobj_NN),
                    "poss_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "vmod", n_all_pobj_NN),
                    "vmod_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "nn", n_all_pobj_NN),
                    "nn_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "rcmod", n_all_pobj_NN),
                    "rcmod_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "advmod", n_all_pobj_NN),
                    "advmod_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "conj_and", n_all_pobj_NN),
                    "conj_and_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(pobj_deps_dict, "conj_or", n_all_pobj_NN),
                    "conj_or_pobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "det", n_all_iobj),
                    "det_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "amod", n_all_iobj),
                    "amod_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "prep", n_all_iobj),
                    "prep_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "poss", n_all_iobj),
                    "poss_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "vmod", n_all_iobj),
                    "vmod_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "nn", n_all_iobj),
                    "nn_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "rcmod", n_all_iobj),
                    "rcmod_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "advmod", n_all_iobj),
                    "advmod_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "conj_and", n_all_iobj),
                    "conj_and_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "conj_or", n_all_iobj),
                    "conj_or_iobj_deps_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "det", n_all_iobj_NN),
                    "det_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "amod", n_all_iobj_NN),
                    "amod_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "prep", n_all_iobj_NN),
                    "prep_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "poss", n_all_iobj_NN),
                    "poss_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "vmod", n_all_iobj_NN),
                    "vmod_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "nn", n_all_iobj_NN),
                    "nn_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "rcmod", n_all_iobj_NN),
                    "rcmod_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "advmod", n_all_iobj_NN),
                    "advmod_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "conj_and", n_all_iobj_NN),
                    "conj_and_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    dict_counter(iobj_deps_dict, "conj_or", n_all_iobj_NN),
                    "conj_or_iobj_deps_NN_struct",
                    index_list,
                    header_list,
                )
            ...
            if clause_check == 1:  # Clause Complexity
                ...
                ktk.dep_counter(
                    lemma_constructicon, None, "cl_av_deps", index_list, header_list
                )
                ktk.std_dev_calc(
                    lemma_constructicon, "cl_ndeps_std_dev", index_list, header_list
                )
                ...
                ktk.dep_counter(
                    lemma_constructicon, "cc", "cc_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "ccomp",
                ...
                ktk.dep_counter(
                    lemma_constructicon, "conj", "conj_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "csubj",
                ...
                ktk.dep_counter(
                    lemma_constructicon, "dep", "dep_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "discourse",
                ...
                ktk.dep_counter(
                    lemma_constructicon, "dobj", "dobj_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon, "expl", "expl_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon, "iobj", "iobj_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon, "mark", "mark_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "ncomp",
                ...
                ktk.dep_counter(
                    lemma_constructicon, "neg", "neg_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "nsubj",
                ...
                ktk.dep_counter(
                    lemma_constructicon, "prt", "prt_per_cl", index_list, header_list
                )
                # ktk.dep_counter(lemma_constructicon, "ref", "ref_per_cl",index_list,header_list)
                ktk.dep_counter(
                    lemma_constructicon, "tmod", "tmod_per_cl", index_list, header_list
                )
                ktk.dep_counter(
                    lemma_constructicon,
                    "xcomp",
            ...
            if soph_check == 1:  # VAC Sophistication
                ...
                ktk.indexer(
                    simple_database_counter(
                        acad_lemma_freq_dict, lemma_constructicon, 0
                    ),
                    "acad_av_lemma_freq",
                    index_list,
                    header_list,
                ...
                ktk.indexer(
                    simple_database_counter(
                        acad_construction_freq_dict, lemma_constructicon, 1
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 3
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 4
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 5
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 6
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 7
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter_log(
                        acad_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter_type(
                        acad_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    ratio_compiler(acad_contingency_dict, lemma_constructicon_no_vcop),
                ...
                ktk.indexer(
                    ratio_compiler_type(
                        acad_contingency_dict, lemma_constructicon_no_vcop
                    ),
                ...
                ktk.indexer(
                    ttr(lemma_constructicon, 0, acad_lemma_freq_dict)[0],
                    "acad_lemma_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, acad_construction_freq_dict)[0],
                    "acad_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, acad_contingency_dict)[0],
                    "acad_lemma_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 0, acad_lemma_freq_dict)[1],
                    "acad_lemma_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, acad_construction_freq_dict)[1],
                    "acad_construction_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, acad_contingency_dict)[1],
                ...
                ktk.indexer(
                    simple_database_counter(
                        news_lemma_freq_dict, lemma_constructicon, 0
                    ),
                    "news_av_lemma_freq",
                    index_list,
                    header_list,
                ...
                ktk.indexer(
                    simple_database_counter(
                        news_construction_freq_dict, lemma_constructicon, 1
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 3
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 4
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 5
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 6
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 7
                    ),
                ...
                ktk.indexer(
                    ratio_compiler(news_contingency_dict, lemma_constructicon_no_vcop),
                ...
                ktk.indexer(
                    ratio_compiler_type(
                        news_contingency_dict, lemma_constructicon_no_vcop
                    ),
                ...
                ktk.indexer(
                    ttr(lemma_constructicon, 0, news_lemma_freq_dict)[0],
                    "news_lemma_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, news_construction_freq_dict)[0],
                    "news_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, news_contingency_dict)[0],
                    "news_lemma_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 0, news_lemma_freq_dict)[1],
                    "news_lemma_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, news_construction_freq_dict)[1],
                    "news_construction_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, news_contingency_dict)[1],
                ...
                ktk.indexer(
                    simple_database_counter(
                        mag_lemma_freq_dict, lemma_constructicon, 0
                    ),
                    "mag_av_lemma_freq",
                    index_list,
                    header_list,
                ...
                ktk.indexer(
                    simple_database_counter(
                        mag_construction_freq_dict, lemma_constructicon, 1
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 3
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 4
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 5
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 6
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 7
                    ),
                ...
                ktk.indexer(
                    ratio_compiler(mag_contingency_dict, lemma_constructicon_no_vcop),
                    "mag_collexeme_ratio",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ratio_compiler_type(
                        mag_contingency_dict, lemma_constructicon_no_vcop
                    ),
                ...
                ktk.indexer(
                    ttr(lemma_constructicon, 0, mag_lemma_freq_dict)[0],
                    "mag_lemma_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, mag_construction_freq_dict)[0],
                    "mag_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, mag_contingency_dict)[0],
                    "mag_lemma_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 0, mag_lemma_freq_dict)[1],
                    "mag_lemma_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, mag_construction_freq_dict)[1],
                    "mag_construction_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, mag_contingency_dict)[1],
                ...
                ktk.indexer(
                    simple_database_counter(
                        fic_lemma_freq_dict, lemma_constructicon, 0
                    ),
                    "fic_av_lemma_freq",
                    index_list,
                    header_list,
                ...
                ktk.indexer(
                    simple_database_counter(
                        fic_construction_freq_dict, lemma_constructicon, 1
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 3
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 4
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 5
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 6
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 7
                    ),
                ...
                ktk.indexer(
                    ratio_compiler(fic_contingency_dict, lemma_constructicon_no_vcop),
                    "fic_collexeme_ratio",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ratio_compiler_type(
                        fic_contingency_dict, lemma_constructicon_no_vcop
                    ),
                ...
                ktk.indexer(
                    ttr(lemma_constructicon, 0, fic_lemma_freq_dict)[0],
                    "fic_lemma_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, fic_construction_freq_dict)[0],
                    "fic_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, fic_contingency_dict)[0],
                    "fic_lemma_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 0, fic_lemma_freq_dict)[1],
                    "fic_lemma_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, fic_construction_freq_dict)[1],
                    "fic_construction_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, fic_contingency_dict)[1],
                ...
                ktk.indexer(
                    simple_database_counter(
                        all_lemma_freq_dict, lemma_constructicon, 0
                    ),
                    "all_av_lemma_freq",
                    index_list,
                    header_list,
                ...
                ktk.indexer(
                    simple_database_counter(
                        all_construction_freq_dict, lemma_constructicon, 1
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon, 0
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 3
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 4
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 5
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 6
                    ),
                ...
                ktk.indexer(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 7
                    ),
                ...
                ktk.indexer(
                    ratio_compiler(all_contingency_dict, lemma_constructicon_no_vcop),
                    "all_collexeme_ratio",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ratio_compiler_type(
                        all_contingency_dict, lemma_constructicon_no_vcop
                    ),
                ...
                ktk.indexer(
                    ttr(lemma_constructicon, 0, all_lemma_freq_dict)[0],
                    "all_lemma_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, all_construction_freq_dict)[0],
                    "all_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, all_contingency_dict)[0],
                    "all_lemma_construction_ttr",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 0, all_lemma_freq_dict)[1],
                    "all_lemma_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    ttr(lemma_constructicon, 1, all_construction_freq_dict)[1],
                    "all_construction_attested",
                    index_list,
                    header_list,
                )
                ktk.indexer(
                    contingency_ttr(lemma_constructicon, all_contingency_dict)[1],
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        all_lemma_freq_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        all_construction_freq_dict, lemma_constructicon, 1, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 3, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 4, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 5, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 6, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon_no_vcop, 7, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        acad_lemma_freq_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        acad_construction_freq_dict, lemma_constructicon, 1, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 3, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 4, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 5, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 6, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon_no_vcop, 7, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        news_lemma_freq_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        news_construction_freq_dict, lemma_constructicon, 1, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 3, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 4, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 5, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 6, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon_no_vcop, 7, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        mag_lemma_freq_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        mag_construction_freq_dict, lemma_constructicon, 1, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 3, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 4, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 5, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 6, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon_no_vcop, 7, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        fic_lemma_freq_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        fic_construction_freq_dict, lemma_constructicon, 1, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon, 0, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 3, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 4, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 5, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 6, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon_no_vcop, 7, "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        all_lemma_freq_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        all_construction_freq_dict, lemma_constructicon, 1, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        all_contingency_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        acad_lemma_freq_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        acad_contingency_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        news_lemma_freq_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        news_contingency_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        mag_lemma_freq_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        mag_construction_freq_dict, lemma_constructicon, 1, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        mag_contingency_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        fic_lemma_freq_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    simple_database_counter(
                        fic_construction_freq_dict, lemma_constructicon, 1, "yes", "yes"
                    ),
                ...
                ktk.std_dev_calc_simple(
                    contingency_database_counter(
                        fic_contingency_dict, lemma_constructicon, 0, "yes", "yes"
                    ),
    ...
    if component_check == 1:
        ...
        comp_out = open(comp_file, "w")
        comp_out.write(
            """filename,np_elaboration,verb_vac_frequency,nouns_as_modifiers,determiners,vac_frequency,association_strength,diversity_and_frequency,possessives,frequency\n"""
        )
        ...
        for components in component_list_dict:
            ...
            variance = list(map(lambda x: (x - mean) ** 2, items))
        ...
        comp_1 = component_counter(
            comp_file_list, component_dict, "Component_1.txt", z_dict
        )
        comp_2 = component_counter(
            comp_file_list, component_dict, "Component_2.txt", z_dict
        )
        comp_3 = component_counter(
            comp_file_list, component_dict, "Component_3.txt", z_dict
        )
        comp_4 = component_counter(
            comp_file_list, component_dict, "Component_4.txt", z_dict
        )
        comp_5 = component_counter(
            comp_file_list, component_dict, "Component_5.txt", z_dict
        )
        comp_6 = component_counter(
            comp_file_list, component_dict, "Component_6.txt", z_dict
        )
        comp_7 = component_counter(
            comp_file_list, component_dict, "Component_7.txt", z_dict
        )
        comp_8 = component_counter(
            comp_file_list, component_dict, "Component_8.txt", z_dict
        )
        comp_9 = component_counter(
            comp_file_list, component_dict, "Component_9.txt", z_dict
        )
    ...
    if sca_check == 1:  # if the SCA box is Checked
        ...
        if system == "M" or system == "L":
            ...
            treg_complex_nominal_1 = (
                "'NP !> NP [<< JJ|POS|PP|S|VBG | << (NP $++ NP !$+ CC)]'"
            )
            treg_complex_nominal_2 = (
        ...
        for files in sca_files:
            ...
            tregex_list = [
                ...
                treg_dep_clause,
                treg_complex_tunit,
                treg_coordinate_phrase,
                treg_complex_nominal_1,
                treg_complex_nominal_2,
                treg_complex_nominal_3,
                treg_frag_clause,
                treg_frag_tunit,
                treg_vp_q,
            ]  # changed in TAASSC 1.1
            tregex_list_count = []
            ...
            for item in tregex_list:
                ...
                tregex_call = subprocess.Popen(
                    tregex_call, shell=True, stdout=subprocess.PIPE
                )
            ...
            nverb_phrase = (
                tregex_list_count[0] + tregex_list_count[11]
            )  # changed in TAASSC 1.1
            ...
            ncomplex_nominal = (
                tregex_list_count[6] + tregex_list_count[7] + tregex_list_count[8]
            )
            ...
            ktk.indexer(
                safe_divide(nverb_phrase, ntunits), "VP_T", index_list, header_list
            )
            ktk.indexer(safe_divide(nclauses, ntunits), "C_T", index_list, header_list)
            ktk.indexer(
                safe_divide(ndep_clause, nclauses), "DC_C", index_list, header_list
            )
            ktk.indexer(
                safe_divide(ndep_clause, ntunits), "DC_T", index_list, header_list
            )
            ktk.indexer(safe_divide(ntunits, nsent), "T_S", index_list, header_list)
            ktk.indexer(
                safe_divide(ncomplex_tunit, ntunits), "CT_T", index_list, header_list
            )
            ktk.indexer(
                safe_divide(ncoordinate_phrase, ntunits),
                "CP_T",
                index_list,
                header_list,
            )
            ktk.indexer(
                safe_divide(ncoordinate_phrase, nclauses),
                "CP_C",
                index_list,
                header_list,
            )
            ktk.indexer(
                safe_divide(ncomplex_nominal, ntunits), "CN_T", index_list, header_list
            )
            ktk.indexer(
                safe_divide(ncomplex_nominal, nclauses), "CN_C", index_list, header_list
            )
    ...
    if sca_check == 1:
        ...
        nfiles = len(p_files_list)

    ######Clean-up:
    folder_list = [
        resource_path("parsed_files/"),
        resource_path("to_process/"),
        resource_path("sca_parsed_files/"),
    ]
