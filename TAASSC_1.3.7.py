...
def main(indir, outfile,check_list):
				...
				if clause_check == 1 or soph_check == 1 or generate_check ==1: #Clause Complexity
					...
					for items in main_verbs:
						...
						if df_check ==1:
							...
							data_file.write(database_string)
	...
	###############			
			if clause_check == 1 or phrase_check ==1 or soph_check ==1:
				...
				##print file_counter
				outf.write ('{0}, {1}, {2}\n'
				.format(filename2,nwords,string))
	...
	if sca_check == 1: #if the SCA box is Checked
		...
		for files in sca_files:
			...
			for item in tregex_list:
				if system == "M" or system == "L":
					tregex_call = "java -mx15g -cp stanford-tregex.jar: edu.stanford.nlp.trees.tregex.TregexPattern "+ item + " " + files + " -C -o"
				if system == "W":
					tregex_call = "java -mx10g -cp "+resource_path("")+"*; edu.stanford.nlp.trees.tregex.TregexPattern "+ item + " " + files + " -C -o"
